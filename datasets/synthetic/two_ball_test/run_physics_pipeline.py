...
def parse_args() -> argparse.Namespace:
    ...
    add_vec3_arg(parser, "--wall-position")
    add_vec3_arg(parser, "--wall-half-extents")
    add_vec3_arg(parser, "--occluder-position")
    add_vec3_arg(parser, "--occluder-half-extents")
...
def apply_scenario_camera_defaults(args: argparse.Namespace) -> None:
    defaults = SCENARIO_CAMERA_DEFAULTS[args.scenario]
    if args.camera_target is None:
        args.camera_target = list(defaults["target"])
    if args.camera_distance is None:
        args.camera_distance = float(defaults["distance"])
    if args.camera_height is None:
        args.camera_height = float(defaults["height"])
    if args.camera_azimuth is None and args.random_view:
        args.camera_azimuth = float(defaults["azimuth"])
    if args.camera_focal_length is None:
        args.camera_focal_length = float(defaults["focal_length"])


def build_generator_cmd(args: argparse.Namespace) -> list[str]:
    if args.scenario != "two_ball_collision":
        cmd = [
            sys.executable,
            str(SCENARIO_GENERATOR),
            "--scenario",
            args.scenario,
            "--out",
            str(args.output_dir),
        ...
        append_optional(cmd, "--ball-radius", args.ball_0_radius if args.ball_0_radius is not None else args.ball_radius)
        append_optional(cmd, "--ball-mass", args.ball_0_mass if args.ball_0_mass is not None else args.ball_mass)
        append_optional(cmd, "--restitution", args.restitution)
        append_optional(cmd, "--friction", args.lateral_friction)
        append_optional(cmd, "--drop-height", args.drop_height)
        append_vec(cmd, "--ball-0-xy", args.ball_0_xy)
        append_vec(cmd, "--ball-0-position", args.ball_0_position)
        append_optional(cmd, "--ball-0-height", args.ball_0_height)
        append_vec(cmd, "--ball-0-velocity", args.ball_0_velocity)
        append_vec(cmd, "--ball-1-xy", args.ball_1_xy)
        append_vec(cmd, "--ball-1-position", args.ball_1_position)
        append_optional(cmd, "--ball-1-height", args.ball_1_height)
        append_vec(cmd, "--ball-1-velocity", args.ball_1_velocity)
        append_vec(cmd, "--wall-position", args.wall_position)
...
def main() -> None:
    args = parse_args()
    apply_scenario_camera_defaults(args)
